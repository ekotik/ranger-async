...
ranger-async - visual file manager
...
B<ranger-async> [B<--version>] [B<--help>] [B<--debug>] [B<--clean>]
...
ranger-async is a console file manager with VI key bindings.
...
This I<manual> contains instructions on how to use and configure ranger-async.

Inside I<ranger-async>, you can press I<?> for a list of key bindings, commands or
settings.
...
installed to F</usr/share/doc/ranger-async/config> and can be obtained with ranger-async's
--copy-config option.

The directory F<examples> contains reference implementations for ranger-async
plugins, sample configuration files and some programs for integrating ranger-async
with other software.  They are usually installed to
F</usr/share/doc/ranger-async/examples>.
...
Activate the debug mode: Whenever an error occurs, ranger-async will exit and print a
...
Activate the clean mode:  ranger-async will not access or create any configuration
...
Change the cache directory of ranger-async from $XDG_CACHE_HOME or ~/.cache/ranger-async to "dir".

=item B<-r> I<dir>, B<--confdir>=I<dir>

Change the configuration directory of ranger-async from $XDG_CONFIG_HOME or ~/.config/ranger-async to "dir".

=item B<--datadir>=I<dir>

Change the data directory of ranger-async from $XDG_DATA_HOME or ~/.local/share/ranger-async to "dir".
...
exporting I<RANGER_ASYNC_LOAD_DEFAULT_RC=FALSE> in your environment.  See also:
...
avoid getting defunct commands on ranger-async upgrades.  The full default
commands.py can be copied with --copy-config=B<commands_full>, but that file
will be ignored by ranger-async and serves only as a reference for making your own
commands.

=item B<--choosefile>=I<targetfile>

Allows you to pick a file with ranger-async.  This changes the behavior so that when
you open a file, ranger-async will exit and write the absolute path of that file into
I<targetfile>.

=item B<--choosefiles>=I<targetfile>

Allows you to pick multiple files with ranger-async.  This changes the behavior so
that when you open a file, ranger-async will exit and write the absolute paths of all
selected files into I<targetfile>, adding one newline after each filename.

=item B<--choosedir>=I<targetfile>

Allows you to pick a directory with ranger-async.  When you exit ranger-async, it will
write the last visited directory into I<targetfile>.

=item B<--selectfile>=I<targetfile>

Open ranger-async with I<targetfile> selected. This is a legacy option, superseded by
...
This part explains how certain parts of ranger-async work and how they can be used
efficiently.
...
This default script is F<%ranger-asyncdir/data/scope.sh>. It contains more
...
%any is used in the ranger-async configuration to create a keybinding for adding a
...
The macro %ranger-asyncdir expands to the directory of ranger-async's python library, you
can use it for something like this command:
 alias show_commands shell less %ranger-asyncdir/config/commands.py
...
Rifle is the file opener of ranger-async.  It can be used as a standalone program or
a python module.  It is located at F<$repo/ranger_async/ext/rifle.py>.  In contrast to
...
rule is used.  In ranger-async, you can list and choose rules by typing "r" or simply
...
F<$repo/ranger_async/config/rifle.conf>.
...
used in the commands C<:open_with> (key "r") and C<:shell> (key "!").
 ...
 f   Fork the process, i.e. run in background. Please use this flag
     ...
     background command when pressing Ctrl+C in ranger-async.
...
you to press Enter before switching back to ranger-async.

=head2 PLUGINS

ranger-async's plugin system consists of python files which are located in
F<~/.config/ranger-async/plugins/> and are imported in alphabetical order when
starting ranger-async.  A plugin changes ranger-asyncs behavior by overwriting or extending
a function that ranger-async uses.  This allows you to change pretty much every part
of ranger-async, but there is no guarantee that things will continue to work in
...
are functions that start with hook_ and can be found throughout the code.
 ...
 grep 'def hook_' -r /path/to/ranger-asyncs/source

Also try:

 pydoc ranger_async.api
...
sample plugins in the F</usr/share/doc/ranger-async/examples/> directory, including a
...
Key bindings are defined in the file F<%ranger-asyncdir/config/rc.conf>.  Check this
...
currently run in ranger-async.  In there, you can type "dd" to abort a process and
"J" or "K" to change the priority of a process.  Only one process is run at a
time.

=item ^C

Stop the currently running background process that ranger-async has started, like
...
This section lists all built-in settings of ranger-async.  The valid types for the
value are in [brackets].  The hotkey to toggle the setting is in <brakets>, if
a hotkey exists.

Settings can be changed in the file F<~/.config/ranger-async/rc.conf> or on the
...
Should ranger-async count and display the number of files in each directory
...
bookmarks between multiple ranger-async instances but leads to *slight* performance
loss.  When false, bookmarks are saved when ranger-async is exited.
...
"always" (default), "never", "multiple". With "multiple", ranger-async will ask only
...
with "j", ranger-async stops scrolling instantly when you release the key.  One
...
combination with B<--choosedir>:
 ...
 ranger-async --choosedir=/tmp/x --cmd='set global_inode_type_filter d'
...
The delay that ranger-async idly waits for user input, in milliseconds, with a
...
=item nested_ranger_async_warning [string]

Warn at startup if C<RANGER_ASYNC_LEVEL> is greater than 0, in other words give a
warning when you nest ranger-async in a subshell started by ranger-async. Allowed values
are C<true>, C<false> and C<error>. The special value C<error> promotes the
warning to an error, this is usually shown as red text but will crash ranger-async
...
use_preview_script is off, ranger-async will handle previews itself by just printing
the content.
...
is reset when you restart ranger-async.
...
Abbreviate $HOME with ~ in the titlebar (first line) of ranger-async?
...
Set the tmux/screen I<window-name> to "ranger-async"?
...
ranger-async.  For your convenience, this is a list of the "public" commands including their parameters, excluding descriptions:
...
functions, one for every method in the ranger-async.core.actions.Actions class.
...
Destroy all files in the selection with a roundhouse kick.  ranger-async will ask for
...
Provides a quick way to view ranger-async documentations.
...
See the I<ranger_async.core.linemode> module for some examples.

=item load_copy_buffer

Load the copy buffer from F<~/.config/ranger-async/copy_buffer>.  This can be used to
pass the list of copied files to another ranger-async instance.
...
displayed when pressing "r" in ranger-async.
...
Save the copy buffer to I<~/.config/ranger-async/copy_buffer>.  This can be used to
pass the list of copied files to another ranger-async instance.
...
Reads commands from a file and executes them in the ranger-async console.

This can be used to re-evaluate the rc.conf file after changing it:

 map X chain shell vim -p %confdir/rc.conf %ranger-asyncdir/config/rc.conf; \
...
to either F<$XDG_DATA_HOME/ranger-async/trash> or F<~/.local/share/ranger-async/trash>.
...
trashed files until this happens. ranger-async will ask for a confirmation if you
...
ranger-async reads several configuration files which are located in
F<$HOME/.config/ranger-async> or F<$XDG_CONFIG_HOME/ranger-async> if $XDG_CONFIG_HOME is
...
F<rc.conf> and F<commands.py> are additionally read from F</etc/ranger-async> if they
...
When starting ranger-async with the B<--clean> option, it will not access or create
any of these files.
...
A python module that defines commands which can be used in ranger-async's console by
...
for custom commands.  It is entirely ignored by ranger-async.
...
file.  In ranger-async, bookmarks can be set by typing m<key>, accessed by typing
...
same files again, pass them to another ranger-async instance or process them in a
script.
...
the path to the file.  In ranger-async, tags can be set by pressing t and removed
...
These environment variables have an effect on ranger-async:

=over 8

=item RANGER_ASYNC_LEVEL

ranger-async sets this environment variable to "1" or increments it if it already
exists.  External programs can determine whether they were spawned from ranger-async
by checking for this variable.

=item RANGER_ASYNC_LOAD_DEFAULT_RC

If this variable is set to FALSE, ranger-async will not load the default rc.conf.
This can save time if you copied the whole rc.conf to I<~/.config/ranger-async/> and
...
Defines the shell that ranger-async is going to use with the :shell command and
the "S" key.  Defaults to "/bin/sh".

=item TERMCMD

Defines the terminal emulator command that ranger-async is going to use with the
...
I<openscad> man page for colorschemes. ranger-async will default to Tomorrow Night.
...
There are various examples on how to extend ranger-async with plugins or combine
ranger-async with other programs.  These can be found in the
F</usr/share/doc/ranger-async/examples/> directory, or the F<doc/ranger-async/> that is
...
=item Download: *TODO: fix link*

=item The project page: *TODO: fix link*

=item The mailing list: *TODO: fix link*

=item IRC channel: *TODO: fix link* on freenode.net

=back

ranger-async is maintained with the git version control system.  To fetch a fresh
copy, run:

 git clone git@github.com:ekotik/ranger-async.git
...
Report bugs here: L<https://github.com/ekotik/ranger-async/issues>

Please include as much relevant information as possible.  For the most
diagnostic output, run ranger-async like this: C<PYTHONOPTIMIZE= ranger-async --debug>
