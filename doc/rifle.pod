...
rifle - ranger-async's file opener
...
written to meet the needs of the file manager I<ranger-async>.  rifle's strength lies
...
rifle shares configuration files with ranger-async, though ranger-async is not required in
order to use rifle. The default configuration file F<rifle.conf> is expected
to be at F<~/.config/ranger-async/rifle.conf>. However, this can be overridden with
...
with ranger-async.  To obtain it, you need to run: C<ranger-async --copy-config=rifle>
