...
The default context tags are specified in `/ranger_async/gui/context.py` in the
constant `CONTEXT_KEYS`. Custom tags can be specified in a custom plugin file in
`~/.config/ranger-async/plugins/`. The code to use follows.

```python
# Import the class
import ranger_async.gui.context

# Add your key names
ranger_async.gui.context.CONTEXT_KEYS.append('my_key')

# Set it to False (the default value)
ranger_async.gui.context.Context.my_key = False

# Or use an array for multiple names
my_keys = ['key_one', 'key_two']
ranger_async.gui.context.CONTEXT_KEYS.append(my_keys)

# Set them to False
for key in my_keys:
    code = 'ranger_async.gui.context.Context.' + key + ' = False'
    exec(code)
```

As you may or may not have guessed, this only tells ranger-async that they exist, not
...
ranger_async.gui.context.CONTEXT_KEYS.append('readme')
ranger_async.gui.context.Context.readme = False
```

Then we'll use the hook `hook_before_drawing` to tell ranger-async that our key is
talking about `README.md` files.

```python
import ranger_async.gui.widgets.browsercolumn

OLD_HOOK_BEFORE_DRAWING = ranger_async.gui.widgets.browsercolumn.hook_before_drawing
...
ranger_async.gui.widgets.browsercolumn.hook_before_drawing = new_hook_before_drawing
```
...
The class `CursesShortcuts` in the file `/ranger_async/gui/curses_shortcuts.py` defines
...
color of directories in the titlebar. This creates a `ranger_async.gui.context.Context`
...
A colorscheme should be a subclass of `ranger_async.gui.ColorScheme` and define the
...
Abbreviations for colors and attributes are defined in `ranger_async.gui.color`. Two
...
- `~/.config/ranger-async/colorschemes/`
- `/path/to/ranger-async/colorschemes/`
...
`~/.config/ranger-async/colorschemes/default.py` or
`/path/to/ranger-async/colorschemes/default.py`.
...
This is done in the jungle colorscheme `ranger_async/colorschemes/jungle`, check it
...
future versions of ranger-async.
