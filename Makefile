...
help:
	@echo 'make:                 Test and compile ranger-async.'
...
compile: clean
	PYTHONOPTIMIZE=$(PYOPTIMIZE) $(PYTHON) -m compileall -q ranger_async

clean:
	find ranger_async -regex .\*\.py[co]\$$ -delete
	find ranger_async -depth -name __pycache__ -type d -exec rm -r -- {} \;
...
TEST_PATHS_MAIN = \
	$(shell find ./ranger_async -mindepth 1 -maxdepth 1 -type d \
		! -name '__pycache__' \
		! -path './ranger_async/config' \
		! -path './ranger_async/data' \
	) \
	./ranger_async/__init__.py \
	$(shell find ./doc/tools ./examples -type f -name '*.py') \
	./ranger_async.py \
	./setup.py \
	./tests
TEST_PATH_CONFIG = ./ranger_async/config
...
test_doctest:
	...
	@set -e; \
	for FILE in $(shell grep -IHm 1 doctest -r ranger_async | grep $(FILTER) | cut -d: -f1); do \
		echo "Testing $$FILE..."; \
		RANGER_ASYNC_DOCTEST=1 PYTHONPATH=".:"$$PYTHONPATH ${PYTHON} $$FILE; \
	done
	@echo
...
test_shellcheck:
	...
	sed '2,$$s/^\([[:blank:]]*\)#/\1/' ./ranger_async/data/scope.sh \
	| shellcheck -a -
	@echo
...
doc/ranger-async.1: doc/ranger-async.pod README.md
	pod2man --stderr --center='ranger-async manual' \
		--date='$(NAME)-$(VERSION)' \
		--release=$(shell date -u '+%Y-%m-%d') \
		doc/ranger-async.pod doc/ranger-async.1
...
man: doc/ranger-async.1 doc/rifle.1

manhtml:
	pod2html doc/ranger-async.pod --outfile=doc/ranger-async.1.html
...
todo:
	@grep --color -Ion '\(TODO\|XXX\).*' -r ranger-async
