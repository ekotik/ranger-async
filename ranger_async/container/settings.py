...
from ranger_async.gui.colorscheme import _colorscheme_name_to_class
...
ALLOWED_SETTINGS = {
    ...
    "mouse_enabled": bool,
    "nested_ranger_async_warning": str,
    "one_indexed": bool,
...
ALLOWED_VALUES = {
    ...
    "draw_borders_multipane": [None, "none", "both", "outline", "separators", "active-pane"],
    "line_numbers": ["false", "absolute", "relative"],
    "nested_ranger_async_warning": ["true", "false", "error"],
...
class Settings(SignalDispatcher, FileManagerAware):
    def __init__(self):
        ...
        for name in ALLOWED_SETTINGS:
            self.signal_bind("setopt." + name, self._sanitize, priority=SIGNAL_PRIORITY_SANITIZE)
            self.signal_bind(
                "setopt." + name, self._raw_set_with_signal, priority=SIGNAL_PRIORITY_SYNC
            )
    ...
    def _sanitize(self, signal):
        ...
        if name == "column_ratios":
            ...
            if not isinstance(value, list) or len(value) < 2:
                ...
                signal.value = [int(i) if str(i).isdigit() else 1 for i in value]
            ...
            if self._settings.get("preview_script") is None and value and self.fm.ui.is_on:
                self.fm.notify("Preview script undefined or not found!", bad=True)
    ...
    def set(self, name, value, path=None, tags=None):
        ...
        assert not (tags and path), "Can't set a setting for path and tag " "at the same time!"
        kws = dict(setting=name, value=value, previous=previous, path=path, tags=tags, fm=self.fm)
    ...
    def get(self, name, path=None):
        ...
        if localpath:
            for pattern, regex in self._localregexes.items():
                if name in self._localsettings[pattern] and regex.search(localpath):
    ...
    def _check_type(self, name, value):
        typ = ALLOWED_SETTINGS[name]
        if isfunction(typ):
            assert typ(value), "Warning: The option `" + name + "' has an incorrect type!"
        else:
