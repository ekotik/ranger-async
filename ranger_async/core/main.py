...
from ranger_async import PY3, VERSION

LOG = getLogger(__name__)

VERSION_MSG = [
    "ranger-async version: {0}".format(VERSION),
...
def main(
    ...
    from ranger_async.container.settings import Settings
    from ranger_async.core.fm import FM
    from ranger_async.core.shared import FileManagerAware, SettingsAware
    from ranger_async.ext.logutils import setup_logging
    from ranger_async.ext.openstruct import OpenStruct

    ranger_async.args = args = parse_arguments()
    ranger_async.arg = OpenStruct(args.__dict__)  # COMPAT
    ...
    # so that programs can know that ranger-async spawned them:
    level = "RANGER_ASYNC_LEVEL"
    ...
    if paths_inaccessible:
        print("Inaccessible paths: {0}".format(", ".join(paths_inaccessible)), file=sys.stderr)
        return 1
    ...
    try:  # pylint: disable=too-many-nested-blocks
        ...
        if args.show_only_dirs:
            from ranger_async.container.directory import InodeFilterConstants

            fm.settings.global_inode_type_filter = InodeFilterConstants.DIRS

        if args.list_unused_keys:
            from ranger_async.ext.keybinding_parser import reversed_special_keys, special_keys
        ...
        if not sys.stdin.isatty():
            sys.stderr.write("Error: Must run ranger-async from terminal\n")
            raise SystemExit(1)
        ...
        if not args.debug:
            from ranger_async.ext import curses_interrupt_handler
        ...
        if int(os.environ[level]) > 1:
            warning = "Warning:"
            nested_warning = "You're in a nested ranger-async instance!"
            warn_when_nested = fm.settings.nested_ranger_async_warning.lower()
        ...
        if ranger_async.args.profile:
            import cProfile
            import pstats

            ranger_async.__fm = fm  # pylint: disable=protected-access
            profile_file = tempfile.gettempdir() + "/ranger_async_profile"
        ...
        exit_msg += """
{0}
ranger-async crashed. Please report this traceback at:
https://github.com/ekotik/ranger-async/issues
""".format(
            ex_traceback
        )
        ...
        if ranger_async.args.profile and profile:
...
def xdg_path(env_var):
    ...
    if path and os.path.isabs(path):
        return os.path.join(path, "ranger-async")
    return None
...
def parse_arguments():
    ...
    from ranger_async import CACHEDIR, CONFDIR, DATADIR, USAGE

    parser = OptionParser(usage=USAGE, version=("\n".join(VERSION_MSG)))

    parser.add_option("-d", "--debug", action="store_true", help="activate debug mode")
    parser.add_option(
        "-c", "--clean", action="store_true", help="don't touch/require any config files. "
    )
    parser.add_option(
        "--logfile", type="string", metavar="file", help="log file to use, '-' for stderr"
    )
    parser.add_option(
        "--cachedir",
        type="string",
    ...
    parser.add_option(
        ...
        type="string",
        metavar="OUTFILE",
        help="Makes ranger-async act like a file chooser. When opening "
    ...
    parser.add_option(
        ...
        type="string",
        metavar="OUTFILE",
        help="Makes ranger-async act like a file chooser for multiple files "
    ...
    parser.add_option(
        ...
        type="string",
        metavar="OUTFILE",
        help="Makes ranger-async act like a directory chooser. When ranger-async quits"
    ...
    parser.add_option(
        ...
        type="string",
        metavar="filepath",
        help="Open ranger-async with supplied file selected.",
    )
    parser.add_option(
        "--show-only-dirs", action="store_true", help="Show only directories, no files or links"
    )
    parser.add_option(
        "--list-unused-keys",
        action="store_true",
    ...
    def path_init(option):
        ...
        try:
            ...
            sys.stderr.write("--{0} is not accessible: {1}\n{2}\n".format(option, argval, str(ex)))
            sys.exit(1)
    ...
    if args.clean:
        ...
        args.cachedir = mkdtemp(suffix=".ranger-async-cache")
        args.confdir = None
        args.datadir = None
...
def load_settings(  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
    ...
    import ranger_async.api.commands
    import ranger_async.core.shared
    from ranger_async.config import commands as commands_default
    from ranger_async.core.actions import Actions

    # Load default commands
    fm.commands = ranger_async.api.commands.CommandContainer()
    ...
    if not clean:
        system_confdir = os.path.join(os.sep, "etc", "ranger-async")
        ...
        if os.environ.get("RANGER_ASYNC_LOAD_DEFAULT_RC", "TRUE").upper() != "FALSE" or not (
    ...
    if allow:
        try:
            ...
            if err.errno != EEXIST:  # EEXIST means it already exists
                ...
                print(confdir)
                print("To run ranger-async without the need for configuration")
