...
class FM(Actions, SignalDispatcher):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, ui=None, bookmarks=None, tags=None, paths=None):
        ...
        self.tabs = {}
        self.tags = tags
        self.restorable_tabs = deque([], ranger_async.MAX_RESTORABLE_TABS)
        self.previews = {}
    ...
    def initialize(self):  # pylint: disable=too-many-statements
        ...
        # directory, keeping the order of files the same as in ranger-async.
    ...
    def copy_config_files(self, which):
        ...
        def copy(src, dest):
            if os.path.exists(self.confpath(dest)):
                ...
                try:
                    os.makedirs(ranger_async.args.confdir)
                except OSError as err:
                    if err.errno != EEXIST:  # EEXIST means it already exists
                        print("This configuration directory could not be created:")
                        print(ranger_async.args.confdir)
    ...
    def confpath(self, *paths):
        """returns path to ranger-async's configuration directory"""
        if ranger_async.args.clean:
            self.notify("Accessed configuration directory in clean mode", bad=True)
            return None
        return os.path.join(ranger_async.args.confdir, *paths)

    def datapath(self, *paths):
        """returns path to ranger-async's data directory"""
        if ranger_async.args.clean:
        ...
        if os.path.exists(path_compat):
            return path_compat
        return os.path.join(ranger_async.args.datadir, *paths)

    @staticmethod
    def relpath(*paths):
        """returns the path relative to ranger-asyncs library directory"""
        return os.path.join(ranger_async.RANGER_ASYNCDIR, *paths)
    ...
    async def loop(self):
        ...
        ranger_async.api.hook_ready(self)
