...
from ranger_async.container.settings import ALLOWED_SETTINGS, ALLOWED_VALUES
from ranger_async.core.loader import CommandLoader, CopyLoader
...
class Actions(  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def notify(self, obj, duration=4, bad=False, exception=None):
        ...
        if isinstance(obj, Exception):
            ...
            exception = obj
            bad = True
        elif bad and ranger_async.args.debug:
    ...
    def abort(self):
        ...
        try:
            ...
            self.notify("Type Q or :quit<Enter> to exit ranger-async")
        else:
    ...
    def execute_file(self, files, **kw):  # pylint: disable=too-many-branches
        ...
        Arguments are the same as for ranger_async.ext.rifle.Rifle.execute:
    ...
    # -- Tags
    # --------------------------
    # Tags are saved in ~/.config/ranger-async/tagged and simply mark if a
    ...
    # -- Bookmarks
    # --------------------------
    # Using ranger_async.container.bookmarks.
