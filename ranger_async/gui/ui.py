...
from ranger_async.ext.keybinding_parser import ALT_KEY, KeyBuffer, KeyMaps
from ranger_async.ext.lazy_property import lazy_property
from ranger_async.ext.signals import Signal
...
def _in_tmux():
    return "TMUX" in os.environ and "tmux" in get_executables()


def _in_screen():
    return "screen" in os.environ["TERM"] and "screen" in get_executables()
...
class UI(  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def setup(self):
        ...
        from ranger_async.gui.widgets.console import Console
        from ranger_async.gui.widgets.pager import Pager
        from ranger_async.gui.widgets.statusbar import StatusBar
        from ranger_async.gui.widgets.taskview import TaskView
        from ranger_async.gui.widgets.titlebar import TitleBar
    ...
    def vcsthread(self):
        """VCS thread"""
        from ranger_async.ext.vcs import VcsThread
    ...
    def draw(self):
        ...
        if self._draw_title and self.settings.update_title:
            ...
            try:
                fixed_cwd = cwd.encode("utf-8", "surrogateescape").decode("utf-8", "replace")
                ...
                for fmt_tup in fmt_tups:
                    sys.stdout.write("%sranger-async:%s%s" % fmt_tup)
                    sys.stdout.flush()
    ...
    def handle_multiplexer(self):
        if self.settings.update_tmux_title and not self._multiplexer_title:
                ...
                if _in_tmux():
                    ...
                    if tmux_allow_rename == "off":
                        ...
                        if self._tmux_automatic_rename == "on":
                            check_output(["tmux", "set-window-option", "automatic-rename", "off"])
                elif _in_screen():
                    # Stores the screen window name before renaming it
                    # gives out a warning if $TERM is not "screen"
                    self._multiplexer_title = check_output(["screen", "-Q", "title"]).strip()
            except CalledProcessError:
                self.fm.notify(
            ...
            if not self._multiplexer_title:
                self._multiplexer_title = os.path.basename(os.environ.get("SHELL", "shell"))

            sys.stdout.write("\033kranger-async\033\\")
            sys.stdout.flush()
    ...
    def _viewmode_to_class(viewmode):
        if viewmode == "miller":
            from ranger_async.gui.widgets.view_miller import ViewMiller

            return ViewMiller
        elif viewmode == "multipane":
            from ranger_async.gui.widgets.view_multipane import ViewMultipane
