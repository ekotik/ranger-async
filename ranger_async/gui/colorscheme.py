...
The values are specified in ranger_async.gui.color.
...
2. be a subclass of ranger_async.gui.colorscheme.ColorScheme
...
Define which colorscheme in your settings (e.g. ~/.config/ranger-async/rc.conf):
...
def _colorscheme_name_to_class(signal):  # pylint: disable=too-many-branches
    # Find the colorscheme.  First look in ~/.config/ranger-async/colorschemes,
    # then at RANGER_ASYNCDIR/colorschemes.  If the file contains a class
