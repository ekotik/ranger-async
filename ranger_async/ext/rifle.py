...
"""rifle, the file executor/opener of ranger-async

This can be used as a standalone program or can be embedded in python code.
When used together with ranger-async, it doesn't have to be installed to $PATH.
...
# Imports from ranger_async library, plus reimplementations in case ranger-async is not
...
try:
    from ranger_async.ext.popen_forked import Popen_forked
except ImportError:
...
class Rifle(object):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, config_file):
        ...
        if __file__.endswith("ranger_async/ext/rifle.py"):
            # Add ranger-async's default mimetypes when run from ranger_async directory
            self._mimetype_known_files.append(__file__.replace("ext/rifle.py", "data/mime.types"))
    ...
    def get_mimetype(self, fname):
        ...
        if not self._mimetype:
            ...
            if self._mimetype == "application/octet-stream":
                try:
                    process = Popen(
                        ["mimetype", "--output-format", "%m", fname], stdout=PIPE, stderr=PIPE
                    )
                    mimetype, _ = process.communicate()
                    self._mimetype = mimetype.decode(ENCODING).strip()
                except OSError:
            ...
            try:
                ...
                if "t" in flags:
                    ...
                    if term not in get_executables():
                        ...
                        self._mimetype = "ranger-async/x-terminal-emulator"
                        self.execute(
                            files=[command.split(";")[1].split("--")[0].strip()] + files,
                            flags="f",
                            mimetype="ranger-async/x-terminal-emulator",
                        )
                        return None
...
def find_conf_path():
    ...
    if "XDG_CONFIG_HOME" in os.environ and os.environ["XDG_CONFIG_HOME"]:
        conf_path = os.environ["XDG_CONFIG_HOME"] + "/ranger-async/rifle.conf"
    else:
        conf_path = os.path.expanduser("~/.config/ranger-async/rifle.conf")
    ...
    if not os.path.isfile(conf_path):
        try:
            # if ranger-async is installed, get the configuration from ranger-async
            import ranger_async
        except ImportError:
            pass
        else:
            conf_path = os.path.join(ranger_async.__path__[0], "config", "rifle.conf")
...
def main():  # pylint: disable=too-many-locals
    ...
    parser.add_option(
        "-w", type="string", default=None, metavar="PROGRAM", help="open the files with PROGRAM"
    )
    parser.add_option(
        "-c",
        type="string",
...
if __name__ == "__main__":
    if "RANGER_ASYNC_DOCTEST" in os.environ:
        import doctest
