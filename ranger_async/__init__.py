...
directory hierarchy.  The secondary task of ranger-async is to figure out which
...
def version_helper():
    if __release__:
        version_string = 'ranger-async {0}'.format(__version__)
    else:
        import subprocess
        version_string = 'ranger-async-master {0}'
        try:
...
# Constants
RANGER_ASYNCDIR = os.path.dirname(__file__)
...
CACHEDIR = os.path.expanduser('~/.cache/ranger-async')
CONFDIR = os.path.expanduser('~/.config/ranger-async')
DATADIR = os.path.expanduser('~/.local/share/ranger-async')

args = None  # pylint: disable=invalid-name

from ranger_async.core.main import main  # NOQA pylint: disable=wrong-import-position
